...
class PlanarImage(ctypes.c_voidp):
    ...
    _LockRect = ctypes.WINFUNCTYPE(ctypes.HRESULT, ctypes.c_uint, ctypes.POINTER(_NuiLockedRect), ctypes.c_voidp,
                                   ctypes.c_uint32)(5, '_LockRect')
    _GetLevelDesc = ctypes.WINFUNCTYPE(ctypes.HRESULT, ctypes.c_uint32, ctypes.POINTER(_NuiSurfaceDesc))(6,
                                                                                                         '_GetLevelDesc')
...
class ImageFrame(ctypes.Structure):
    _fields_ = [('timestamp', ctypes.c_longlong),
                # The timestamp (in milliseconds) of the most recent frame. The clock starts when you call Initialize.
...
class SkeletonBoneOrientation(ctypes.Structure):
    ...
    def __repr__(self):
        return '<SkeletonBoneOrientation(%r, %r, %r, %r)>' % (
        self.end_joint, self.start_joint, self.hierarchical_rotation, self.absolute_rotation)
